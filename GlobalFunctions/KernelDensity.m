    ...
    for j = 1 : length(values_X),
        ...
        for i = 1 : length(index_Y),
            for k = 1 : size(dataset_modif,1),
                tmp(index_Y(i),j) = tmp(index_Y(i),j) + pdf_compute(values_X(j),values(index_Y(j)),dataset_modif(k,1),dataset_modif(k,2));
            end
        end
    end
    Dist = sum(tmp,2);
...
min_compute = min(dataset(:,2))-4*bandwidth_Y;
max_compute = max(dataset(:,2))+4*bandwidth_Y;
