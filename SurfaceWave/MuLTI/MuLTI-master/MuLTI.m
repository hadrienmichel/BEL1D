...
load('testModel.mat'); %load picked dispersion curves 
...
fitting_error = 80;%half_width(:,2); % set fitting error to half width of waveform's dispersion image
...
nsample=100000; % total number of samples
...
num_layers = 2; % depth constraining parameter, if set to 1 no depth constraints will be applied.
...
elseif num_layers == 2
    priors.layer_depths(1) = 5 ; % layer 1 depth.
    priors.vsmin(1) = 200; % layer 1
    priors.vsmax(1) = 1700;
    priors.vsmin(2) = 500; % layer 3
    priors.vsmax(2) = 2800;
    priors.vp(1) = 600; % layer 1 fixed vp    
    priors.vp(2) = 2000; % layer 3 fixed Vp
    priors.density(1) = 1.6; % layer 1 fixed density    
    priors.density(2) = 2.0; % layer 3 fixed density
else % num_layers = 1 
